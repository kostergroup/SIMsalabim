...
CONST
    version = '3.77';   {version, 1.00 = 10-03-2004}
...
{**Contacts**************************************************************************}
	...
	Get_Float(inv, msg, 'SnMin', SnMin); {m/s, surface recombination of electrons at anode}
	Get_Float(inv, msg, 'SpMin', SpMin); {m/s, surface recombination of holes at cathode}
	Get_Float(inv, msg, 'SnMaj', SnMaj); {m/s, surface recombination of electrons at anode, majority carriers}
	Get_Float(inv, msg, 'SpMaj', SpMaj); {m/s, surface recombination of holes at cathode, majority carriers}
...
{**Trapping**************************************************************************}
    ...
    {** Bulk traps}
    Get_Float(inv, msg,'Bulk_tr', Bulk_tr); {density of electron or hole traps, m^-3}
    IF Bulk_tr >=0
	THEN BEGIN
		Pt:=Bulk_tr;
		Nt:=0;
	...
	END;
    {** Surface traps}
    Get_Float(inv, msg, 'LTL', LTL); {m, thickness left trap layer}
    Get_Float(inv, msg, 'LTR', LTR); {m, thickness right layer}
    Get_Float(inv, msg, 'St_L', St_L); {m^-2, density of left surface traps}
    Get_Float(inv, msg, 'St_R', St_R); {m^-2, density of right surface traps}
    {** Grain boundaries}
    ...
    Get_Float(inv, msg, 'L_GB', L_GB); {m, thickness of the grain boundary}
    {** traps coefficients}
    Get_Float(inv, msg, 'Cn', Cn); {m^3/s, capture coefficient for electrons (put to 0 to exclude SRH)}
    Get_Float(inv, msg, 'Cp', Cp); {m^3/s, capture coefficient for holes (put to 0 to exclude SRH) }
    Get_Integer(inv, msg, 'Trtype', dumint);
    Trtype:=ROUND(dumint); {trap type electrons 0: single level, 1 : exponential}
    Get_Float(inv, msg, 'Etrap', Etrap); {eV, electron trapping level, w.r.t. conduction band}
    Get_Float(inv, msg, 'Ttr', Ttr); {K, charac. temperature of exponential distr. traps}
    Get_Integer(inv, msg, 'Q_charge', dumint);
...
BEGIN
	...
	Vstep:=ABS(JVexp[2].Vint - JVexp[1].Vint);
	IF JVexp[2].Vint > JVexp[1].Vint THEN Vscan:=1 ELSE Vscan:=-1; {determine scan direction}
	...
	FOR i:=1 to NJV-1 DO
		IF ABS(JVexp[i+1].Vint - JVexp[i].Vint - Vscan * Vstep) > tolReal {comparing reals, hence the tolerance}
