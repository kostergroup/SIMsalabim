...
procedure UpdateTo529(ProgName : string; var version : integer; SetupFile : string);
{updates the setup file (SetupFile) and layer files (LayerFile) by 1 version number}
var inp, outp : text;
	line, newVersionString, LayerFile : string;
	dum1, dum2 : string;
	...
	procedure UpdateLayerFile529(path, LayerFile : string); 
	{only works on a single layer file}
	var inp, outp : text;
		buffer : ansistring;
		line, dumstr : string;
...
begin
	if version=528 then {we only update 1 version number!}
	begin
		path:=ExtractFilePath(SetupFile); {we need the path to get to the right LayerFile!}
...
{till here}
			...
			begin
				...
				GetProgNameVersion(dum1, LayerVersion, path + LayerFile); 
				if LayerVersion = version then {only update the layer file if we need to!}
					UpdateLayerFile529(path, LayerFile); 
...
procedure UpdateTo530(ProgName : string; var version : integer; SetupFile : string);
{updates the setup file (SetupFile) and layer files (LayerFile) by 1 version number}
var inp, outp : text;
	line, newVersionString, LayerFile : string;
	dum1, dum2 : string;
	...
	procedure UpdateLayerFile530(path, LayerFile : string); 
	{only works on a single layer file}
	var inp, outp : text;
		buffer : ansistring;
		line, dumstr : string;
...
begin
	if version=529 then {we only update 1 version number!}
	begin
		path:=ExtractFilePath(SetupFile); {we need the path to get to the right LayerFile!}
...
{till here}
			...
			begin
				...
				GetProgNameVersion(dum1, LayerVersion, path + LayerFile); 
				if LayerVersion = version then {only update the layer file if we need to!}
					UpdateLayerFile530(path, LayerFile); 
...
procedure UpdateTo531(ProgName : string; var version : integer; SetupFile : string);
{updates the setup file (SetupFile) and layer files (LayerFile) by 1 version number}
var inp, outp : text;
	line, newVersionString, LayerFile : string;
	dum1, dum2 : string;
	...
	procedure UpdateLayerFile531(path, LayerFile : string); 
	{only works on a single layer file}
	var inp, outp : text;
		buffer : ansistring;
		line, dumstr : string;
...
begin
	if version=530 then {we only update 1 version number!}
	begin
		path:=ExtractFilePath(SetupFile); {we need the path to get to the right LayerFile!}
...
{till here}
			...
			begin
				...
				GetProgNameVersion(dum1, LayerVersion, path + LayerFile); 
				if LayerVersion = version then {only update the layer file if we need to!}
					UpdateLayerFile531(path, LayerFile); 
...
procedure UpdateTo532(ProgName : string; var version : integer; SetupFile : string);
{updates the setup file (SetupFile) and layer files (LayerFile) by 1 version number}
var inp, outp : text;
	line, newVersionString, LayerFile : string;
	dum1, dum2 : string;
	...
	procedure UpdateLayerFile532(path, LayerFile : string); 
	{only works on a single layer file}
	var inp, outp : text;
		buffer : ansistring;
		line, dumstr : string;
...
begin
	if version=531 then {we only update 1 version number!}
	begin
		path:=ExtractFilePath(SetupFile); {we need the path to get to the right LayerFile!}
...
{till here}
			...
			begin
				...
				GetProgNameVersion(dum1, LayerVersion, path + LayerFile); 
				if LayerVersion = version then {only update the layer file if we need to!}
					UpdateLayerFile532(path, LayerFile); 
...
begin
	...
	UpdateTo529(ProgName, version, SetupFile);
